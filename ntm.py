...
class NTMCell(rnn_cell.RNNCell):
  ...
  def conv_sum(wg_i, shift_i, row_idx):
    ...
    def conv_step(i, wg_i, shift_i, row_idx, total): 
      si = (row_idx - i) % tf.shape(shift_i)[0]
      total += wg_i[i] * shift_i[si]
      i += 1  
      return i, wg_i, shift_i, row_idx, total
    (_, _, _, _, total) = tf.while_loop(
  ...
  def rotate(wg, shift):
    ...
    def rotate_step(i, wg, shift, ws):
      batch_idx = i / tf.shape(wg)[0]
      row_idx = i % tf.shape(wg)[0]
      wg_i = tf.slice(wg, [batch_idx, 0], [1, -1])
      shift_i = tf.slice(shift, [batch_idx, 0], [1, -1])
      ws_i = conv_sum(wg_i, shift_i, row_idx)
      ws.write(i, ws_i)
      i += 1
      return i, wg, shift, ws
    i = tf.constant(0)
    (_, _, _, ws) = tf.while_loop(
      cond=lambda i, wg, _1, _2: i < tf.reduce_prod(tf.shape(wg)),
      body=rotate_step,
      loop_vars=(i, wg, shift, ws),
    )
    ws = tf.reshape(ws.pack(), [tf.shape(wg)[0], tf.shape(wg)[1]])
    return ws
